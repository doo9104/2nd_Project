...
public class BoardDisplayApiController {
    ...
    public String showList(@PageableDefault(
            size = 6 ) Pageable pageable, Model model, @PathVariable BoardType type) {
    ...
    public String post(@PathVariable BoardType type) {
        return "/board/post";
    }

    // 게시판에 맞춰서 쿠키 이름 재설정
    @GetMapping("/{type}/{id}")
    public String view(@PathVariable Long id, @PathVariable BoardType type,Model model, HttpServletResponse response, HttpServletRequest request) {
    ...
    public String updateView(@PathVariable Long id, @PathVariable BoardType type, Model model) {
        BoardPostResponseDto dto = board_service.findById(id);
